...
- **Placeholders**: If generating placeholders, use colored rectangles and add a comment: `// TODO: Generate asset via Nano Banana: [Prompt description].`

## Mobile First
- **Touch Events**: Every game MUST include touch controls (touchstart, touchmove, touchend) in addition to keyboard/mouse event listeners.
...
- **No Popups**: NEVER use `alert()`, `prompt()`, or `confirm()`. Create custom HTML overlays for game over / pause screens.
